...
void SysTick_Init(uint32_t period) {
	...
	SCB->SHP[3] = (SCB->SHP[3]&0x1FFFFFFF)|0x50000000;	// priority 2
...
void SysTick_Handler(void){
	//index = (index + 1)&0x1F;
	DAC_Out(wave[index]);
	int num = (int)wave[(int)index];
	if (((int)Counts) == (31*110))
	{
	    uint8_t in = P5IN&0x07;             //Check input after wave is finished
	    incheck = in&check;                 //
	    //uint8_t inxor = in^check;
	    if(in != 0)
	    {
	        if ((P5IN&0x07) == check){
	            Counts = 0;
